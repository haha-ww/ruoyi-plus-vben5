...
import { crmCustomerList, crmCustomerRemove,crmCustomerExport } from '#/api/crm/crmCustomer';
...
const { exportBlob, exportLoading, buildExportFileName } =
  useBlobExport(crmCustomerExport);

async function handleDownloadExcel() {
  const formValues = await tableApi.formApi.getValues();
  const fileName = buildExportFileName('客户-客户档案数据');
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="客户-客户档案列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['crm:crmCustomer:export']"
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['crm:crmCustomer:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['crm:crmCustomer:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
      ...
      <template #action="{ row }">
        <Space>
          <action-button
            v-access:code="['crm:crmCustomer:edit']"
