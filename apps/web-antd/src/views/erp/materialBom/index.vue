...
const [MaterialBomModal, modalApi] = useVbenModal({
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: Required<MaterialBomForm>) => row.id);
  window.modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await materialBomRemove(ids);
      await tableApi.query();
    },
  });
}

const { exportBlob, exportLoading, buildExportFileName } = useBlobExport(materialBomExport);

async function handleExport() {
  const formValues = await tableApi.formApi.getValues();
  const fileName = buildExportFileName('物料bom数据');
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="物料bom列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['erp:materialBom:export']"
          ...
          <a-button
            type="primary"
            v-access:code="['erp:materialBom:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
