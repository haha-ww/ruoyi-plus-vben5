...
const gridOptions: VxeGridProps = {
  ...
  // 表格全局唯一表示 保存列配置需要用到
  id: 'erp-technologyRouting-index',
};
...
const [TechnologyRoutingModal, modalApi] = useVbenModal({
  connectedComponent: technologyRoutingModal,
});
...
async function handleDelete(row: Required<TechnologyRoutingForm>) {
  await technologyRoutingRemove(row.id);
  await tableApi.query();
}

function handleMultiDelete() {
  const rows = tableApi.grid.getCheckboxRecords();
  const ids = rows.map((row: Required<TechnologyRoutingForm>) => row.id);
  window.modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await technologyRoutingRemove(ids);
      await tableApi.query();
    },
  });
}

const { exportBlob, exportLoading, buildExportFileName } = useBlobExport(technologyRoutingExport);

async function handleExport() {
  const formValues = await tableApi.formApi.getValues();
  const fileName = buildExportFileName('erp-工艺路线数据');
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="工艺路线列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['erp:technologyRouting:export']"
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['erp:technologyRouting:remove']"
          ...
          <a-button
            type="primary"
            v-access:code="['erp:technologyRouting:add']"
            @click="handleAdd"
          >
            {{ $t('pages.common.add') }}
          </a-button>
    ...
    <TechnologyRoutingModal @reload="tableApi.query()" />
  </Page>
</template>
