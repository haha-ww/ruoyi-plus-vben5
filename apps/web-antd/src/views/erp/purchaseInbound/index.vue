  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        return await purchaseInboundList({
...
const [PurchaseInboundModal, modalApi] = useVbenModal({
  connectedComponent: purchaseInboundModal,
});
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: Required<PurchaseInboundForm>) => row.id);
...
const { exportBlob, exportLoading, buildExportFileName } = useBlobExport(purchaseInboundExport);

async function handleExport() {
  const formValues = await tableApi.formApi.getValues();
  const fileName = buildExportFileName('采购入库数据');
...
<template>
  <Page :auto-content-height="true">
    <BasicTable table-title="采购入库列表">
      <template #toolbar-tools>
        <Space>
          <a-button
            v-access:code="['erp:purchaseInbound:export']"
    ...
    <PurchaseInboundModal @reload="tableApi.query()" />
  </Page>
</template>
