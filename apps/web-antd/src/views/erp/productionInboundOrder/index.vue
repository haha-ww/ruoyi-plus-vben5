...
const gridOptions: VxeGridProps = {
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues = {}) => {
        return await productionInboundOrderList({
  ...
  // 表格全局唯一表示 保存列配置需要用到
  id: 'erp-productionInboundOrder-index',
};
...
const [ProductionInboundOrderModal, modalApi] = useVbenModal({
  connectedComponent: productionInboundOrderModal,
});
...
async function handleEdit(row: Required<ProductionInboundOrderForm>) {
...
function handleMultiDelete() {
  ...
  const ids = rows.map((row: Required<ProductionInboundOrderForm>) => row.id);
  window.modal.confirm({
    title: '提示',
    okType: 'danger',
    content: `确认删除选中的${ids.length}条记录吗？`,
    onOk: async () => {
      await productionInboundOrderRemove(ids);
      await tableApi.query();
    },
  });
}

const { exportBlob, exportLoading, buildExportFileName } = useBlobExport(productionInboundOrderExport);

async function handleExport() {
  const formValues = await tableApi.formApi.getValues();
  const fileName = buildExportFileName('生产入库数据');
...
<template>
  <Page :auto-content-height="true">
    ...
    <ProductionInboundOrderModal @reload="tableApi.query()" />
  </Page>
</template>
